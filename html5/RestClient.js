...
// Model for People entity
    models.People = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/demo/webresources/com.leo.demo.people/",
        idAttribute: 'id',
        defaults: {
            birthday: "",
            address: "",
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/demo/webresources/com.leo.demo.people/';
            }
    ...
    // Collection class for People entities
    models.PeopleCollection = Backbone.Collection.extend({
        model: models.People,
        url: "http://localhost:8080/demo/webresources/com.leo.demo.people/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
            ...
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        ...
                        // tpl-people-list-itemis template identifier for item
                        templateName: '#tpl-people-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.people = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.people,
                    // tpl-people-details is template identifier for chosen model element
                    templateName: '#tpl-people-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.People();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-people-details is a template identifier for chosen model element
                templateName: '#tpl-people-details',
